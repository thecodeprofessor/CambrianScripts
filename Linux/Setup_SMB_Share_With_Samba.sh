...
sudo docker run -it --name smbserver -v ~/smb/shares/public:/shares/public -v ~/smb/shares/departments/accounting:/shares/departments/accounting -v ~/smb/shares/users/user1:/shares/users/user1 -v ~/smb/shares/users/user2:/shares/users/user2 -p 139:139 -p 445:445 -e TZ=EST5EDT -d dperson/samba -p -u "user1;supersecret" -u "user2;extrasecret" -s "public;/shares/public" -s "accounting;/shares/departments/accounting;yes;no;no;user1,user2" -s "user1;/shares/users/user1;yes;no;no;user1" -s "user2;/shares/users/user2;yes;no;no;user2"
